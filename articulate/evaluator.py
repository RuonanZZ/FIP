...
class RotationErrorEvaluator:
    ...
    def __call__(self, p: torch.Tensor, t: torch.Tensor):
        ...
        """
        return radian_to_degree(angle_between(p, t, self.rep).abs().mean(dim=0).unsqueeze(0))
...
class PerJointRotationErrorEvaluator:
    ...
    def __call__(self, p: torch.Tensor, t: torch.Tensor, joint_num, return_each=False):
        ...
        return torch.cat([radian_to_degree(angle_between(p[:,i,:], t[:,i,:], self.rep).abs().mean(dim=0).unsqueeze(0)) for i in range(joint_num)], dim=0)
